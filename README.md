...
The instruction memory stores the program instructions that the processor executes. It acts as a read-only memory (ROM) in this design, with preloaded instructions.
...
**Role in Processor:** 
		The register file provides operands to the ALU (via rs1_data and rs2_data) and stores the results of ALU operations (via rd_data). It is part of the execute and write-back stages of the execution cycle.
...
**Role in Processor:** 
			The ALU performs the core computation of the processor (ADD or SUB) and produces the result, which is then written back to the register file. It is part of the execute stage of the execution cycle.
...
**Role in Processor:** 
		This module is the heart of the processor, orchestrating the fetch-decode-execute-write-back cycle. It connects all submodules and ensures data flows correctly between them.
...
**Role in Processor:** 
		The testbench verifies that the processor works as expected by running the sample program and checking the final result. It also generates a VCD file for detailed waveform analysis.
